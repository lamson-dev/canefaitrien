...
public class Controller implements IGameModel {
	 ...
	 */
	public Controller(Person player, Ship ship, Planet location, int money,
			Universe universe, Difficulty difficulty, int turn) {
		this.player = player;
		this.ship = ship;
	...
	public Controller(Person player, Difficulty difficulty) {
		this(player, new Ship(ShipType.GNAT), null, STARTING_MONEY,
				new Universe(), difficulty, 0);
	...
	public void move(Planet destination) throws Exception {
		if (location.distance(destination) > ship.getType().MAX_DISTANCE) { // if
																			// planet
																			// is
																			// too
																			// far
			throw new Exception("That planet is too far, captain!");
		} else if (location.distance(destination) > ship.getFuel()) { // if not
																		// enough
																		// fuel
